...
Public site, online appointments, booking notifications by SMS and email, a diagnostics price list, a bilingual staff panel and a patient portal — all of it in English and বাংলা. 290 feature tests.
...
- **Site controls** (`/admin/site-controls`) — a switchboard for the public site: every area (departments, doctors, packages, diagnostics, the health hub, the portal, booking), every band of the home page, the header and footer furniture, and what visitors may do (online booking, the contact form, portal sign-ups, online payment, stand-in photography, motion, maintenance mode). Switching an area off hides every link to it **and** closes its URL, so an old bookmark cannot walk in behind the navigation. Staff signed in to the panel keep seeing the whole site, so a page can be checked before it goes back on air
- **Stand-in photography** — consultant portraits, department headers and article covers are image-led, and any slot with no upload falls back to a stand-in hospital photograph, matched to the consultant's recorded gender and stable for the life of the row. A real upload always wins, the panel labels a preview that is only a stand-in, and the whole fallback is one switch away from off
...
290 feature tests covering page rendering, doctor search, the contact form, the appointment booking flow (including double-booking, out-of-schedule and out-of-window rejection), UI localisation (persistence, fallback, allow-list enforcement, date localisation, exact key + placeholder parity), content localisation (full Bangla coverage on every record, per-locale setting cache, and both-script search), the staff panel (every route guarded, login throttling, translation writes and clears, slug generation, image upload/replace/remove, chamber-hour overlap rejection, front-desk booking, and the delete guards that protect existing records), and the notifications (who gets an email or an SMS and who does not, queued rather than sent inline, the patient's language surviving a staff member working in the other one, number normalisation, gateway responses that say 200 OK while failing, SMS template segment budgets, and neither a dead mail server nor a dead gateway breaking a booking), the day-before reminder (confirmed bookings only, never twice, dry runs, and the idempotence that lets a failed run be repeated by hand), the diagnostics catalogue (search by name and code, category filtering, hidden tests staying unreachable, and test requests reaching the inbox with the price attached), the patient portal (guards that keep patients out of the panel, appointments matched across every spelling of a mobile number, single-use expiring reset codes, and documents that one patient can never read from another's account), the site controls (defaults that leave the site untouched, a switched-off area answering 404 to visitors and 200 to staff, sections hidden without closing their area, booking closed while its page stays up, maintenance mode returning 503 with the emergency numbers while the panel and portal stay open, and a save that writes every switch rather than only the ticked ones), and the stand-in photography (every declared file present on disk, stable and non-repeating picks, portraits that never cross gender pools, uploads winning, and the whole thing switching off cleanly).
